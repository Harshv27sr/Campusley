      ...
      <section className="section-padding bg-white dark:bg-slate-900">
        <div className="container-xl">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold font-display text-slate-900 dark:text-white">Top <span className="gradient-text">Contributors</span></h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => <div key={i} className="h-20 bg-slate-100 dark:bg-slate-800 rounded-2xl animate-pulse" />)}
          </div>
        </div>
      </section>
    )
            ...
            {[contributors[1], contributors[0], contributors[2]].map((c, i) => {
              ...
              return (
                ...
                >
                  ...
                  <Avatar src={c.avatar} name={c.name} size="lg" className="mb-2 ring-4 ring-white dark:ring-slate-900" />
          ...
          {contributors.slice(3).map((c, i) => (
            ...
            >
              <Link to={`/profile/${c._id || c.id}`}>
                <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all hover:shadow-md group">
                  <span className="text-2xl">{badges[i + 3] || '🌟'}</span>
                  <Avatar src={c.avatar} name={c.name} size="md" />
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-900 dark:text-white text-sm truncate group-hover:text-indigo-600 transition-colors">{c.name}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{c.college || c.schoolName || 'Student'}</p>
                  </div>
